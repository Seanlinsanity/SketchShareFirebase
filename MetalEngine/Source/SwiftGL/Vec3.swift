...
public func smoothstep(a: Vec3, b: Vec3, t: Float) -> Vec3 {return mix(a:a, b: b, t: t * t * (3 - 2 * t))}

public func clamp(value: Vec3, min: Vec3, max: Vec3) -> Vec3 {return Vec3(clamp(value:value.x, min: min.x, max: max.x), clamp(value: value.y, min: min.y, max: max.y), clamp(value: value.z, min: min.z, max: max.z))}
public func mix(a: Vec3, b: Vec3, t: Vec3) -> Vec3 {return a + (b - a) * t}
public func smoothstep(a: Vec3, b: Vec3, t: Vec3) -> Vec3 {return mix(a:a, b: b, t: t * t * (Vec3(s: 3) - 2 * t))}
