...
public func clamp(value: Vec4, min: Float, max: Float) -> Vec4 {return Vec4(clamp(value:value.x, min: min, max: max), clamp(value: value.y, min: min, max: max), clamp(value: value.z, min: min, max: max), clamp(value: value.w, min: min, max: max))}
public func mix(a: Vec4, b: Vec4, t: Float) -> Vec4 {return a + (b - a) * t}
public func smoothstep(a: Vec4, b: Vec4, t: Float) -> Vec4 {return mix(a:a, b: b, t: t * t * (3 - 2 * t))}

public func clamp(value: Vec4, min: Vec4, max: Vec4) -> Vec4 {return Vec4(clamp(value:value.x, min: min.x, max: max.x), clamp(value: value.y, min: min.y, max: max.y), clamp(value: value.z, min: min.z, max: max.z), clamp(value: value.w, min: min.w, max: max.w))}
public func mix(a: Vec4, b: Vec4, t: Vec4) -> Vec4 {return a + (b - a) * t}
public func smoothstep(a: Vec4, b: Vec4, t: Vec4) -> Vec4 {return mix(a:a, b: b, t: t * t * (Vec4(s: 3) - 2 * t))}
